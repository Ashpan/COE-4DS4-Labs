...
status_t SPI_write(uint8_t regAddress, uint8_t value) {
    ...
    masterTxData[1] = regAddress & 0x80; //Clear the least significant 7 bits
    masterTxData[2] = value; //Clear the least significant 7 bits
    masterXfer.txData = masterTxData;
...
int main(void)
{
    ...
    for(volatile int i = 0U; i < 100000; i++) {
    	__asm("NOP");
    }

    SPI_read(0x12, &byte, 1);
    printf("Expected 0x0 and got 0x%X\n", byte);
    SPI_write(0x12, 0x21);
    SPI_read(0x12, &byte, 1);
	printf("Expected 0x21 and got 0x%X\n", byte);
